...
\subtitle{An Offline Programming Framework for Externally Guided Motion and Virtual Sensors in RobotStudio}
...
\sammanfattning{Några ord!}

\nyckelord{Några, Ord, Till}
...
\subsection{Default frames for position, orientation in RobotWare}
...
\begin{figure}[H]
    \begin{center}
    \tikzset{every picture/.append style={transform shape, scale=0.65}}
...
\begin{figure}[H]
    ...
    \hline
\tiny{\textbf{Name}} & \tiny{\textbf{Type}} & \tiny{\textbf{Serial Port}} & \tiny{\textbf{Remote Address}} & \tiny{\textbf{Remote Port Number}}\\ \hline
...
// The serialization file (egm.cs) was created from egm.proto
// and its namespace abb.egm has EGM message handeling code.
using abb.egm;      

// Receive a byte[] from any IP address on a port for this endpoint
...
\subsection{EgmSensor: movement instruction message protocol}
...
\subsection{EgmSensorPathCorr: correction message protocol}
...
\begin{enumerate}
    \item An EGM endpoint listening at localhost port: 8080 
    \item A virtual controller named ROB\_1 in RobotStudio
    \item RobotWare-option for EGM enabled for ROB\_1 
    \item The transmission protocol presented in section \ref{sec:EGM:Process_Management} defined for ROB\_1: 
    \item The 'default' EGM process configuration (see section \ref{sec:EGM:Process_Management})
\end{enumerate}
...
\begin{figure}[H]
    \centering
    \begin{lstlisting}
! This example assumes the definition of these targets elsewhere.
...
PROC Stream()
    ! Instantiates an EGM process (egmProc)
    EGMGetId egmProc;
    ! Starts egmProc with 'default' configuration and 
    ! instructs the process to communicate with:
    !   - the motion control process of ROB_1 and
    !   - the EGM endpoint located at 'endpoint_address'
    EGMSetupUC ROB_1, egmProc, "default", "endpoint_address"\Joint;
    ! Instructs egmProc to stream with a sample rate of 16 ms
    EGMStreamStart egmProc\SampleRate:=16;
    ! Run Path_10 procedure
    Path_10;
    ! Instructs egmProc to stop streaming
...
\begin{figure}[H]
    \begin{center}
    \tikzset{every picture/.append style={transform shape, scale=0.85}}
    \begin{sequencediagram}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=8cm]{pic_process-state-diagram.png}
...
Once the fine movement to a start position is complete, the connection setup can begin. Although it should not happen in this example program, it is possible that a given EGM process could still be in a state of {\scriptsize{EGM\_STATE\_RUNNING}} in a more complicated program. It is recommended good practice to reset EGM processes (EGMReset egmProc) before using them, but not required. The instruction 'EGMGetId egmProc' will instantiate an EGM process if one is not already bound to the name 'egmProc'. The function 'EGMGetState' will return the state of the EGM process. The condition that 'egmSt $<=$ {\scriptsize{EGM\_STATE\_CONNECTED}}' will be true if the state is {\scriptsize{EGM\_STATE\_DISCONNECTED}} or {\scriptsize{EGM\_STATE\_CONNECTED}} but will return false if the sate is {\scriptsize{EGM\_STATE\_RUNNING}}. Because this program forces a reset, the state will be {\scriptsize{EGM\_STATE\_DISCONNECTED}}. The connection to the EGM endpoint is setup with the instruction 'EGMSetupUC' in the same way as for position streaming. The option \textbackslash Pose is used here instead of the option \textbackslash Joint that was used in the position stream example. All this does is establish the kind of movement instructions the robot controller will receive from the EGM endpoint. Establishing pose here means that the EGM process will be expecting movement instructions in the form of position and orientation (see section \ref{sec:Tech_Back:Rob_Move:pos_orient}). \par
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{pic_endpoint-process-motion-diagram.png}
...
The general sequence of events in path correction is similar to those of position streaming. I.e. both EGM processes run concurrently to the execution of a path that is defined in the RAPID program. Where they differ is in the messages that are received from the EGM endpoint and what is done with the messages. In position streaming, EgmSensor messages are sent from the endpoint and disregarded by the controller. In path correction, the endpoint sends EgmSensorPathCorr messages to the controller which are not ignored. The EgmSensorPathCorr messages contain $y$ and $z$ values for a translation of the TCP in reference to a 'correction frame'. In figure \ref{fig:rapid_path_corr}, the EGM process 'egmProc' is setup for 'ROB\_1' with 'pathCorr' parameters and transmission protocol 'endpoint\_address' by the instruction 'EGMSetupUC'. The EGM process is then started with 'EGMActMove'. This begins the data stream to the EGM endpoint. When the first move instruction is called (i.e. MolveL Home) the endpoint receives EgmRobot messages, but any EgmSensorPathCorr messages sent are ignored. Only when the second move instruction (i.e. EGMMoveL egmProc, Target\_10) is called does the robot controller begin to take the EgmSensorPathCorr messages into account. While executing one of these EGMMoveL instructions, The actual position of the TCP can be displaced in the $yz$-plane of the correction frame (see section \ref{sec:EGM:Modes:corr}). 
...
PROC runPathCorr()
    ...
    MoveL Home, v100, fine, tool0 \WObj:=wobj0;
    EGMMoveL egmProc, Target_10, v100, z10, tool0 \WObj:=wobj0;
    EGMMoveL egmProc, Target_20, v100, fine, tool0 \WObj:=wobj0;
    MoveL Home, v100, fine, tool0 \WObj:=wobj0;
    EGMReset egmProc;
ENDPROC
    \end{lstlisting}
...
\begin{figure}[H]
 \begin{center}
    \tikzset{every picture/.append style={transform shape, scale=1.65}}
\tdplotsetmaincoords{70}{110}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=8cm]{Demo_markup.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=10cm]{ArchComponents2.jpg}
